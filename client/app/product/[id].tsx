import { View, Text, ActivityIndicator, ScrollView, Image, Dimensions, Touchable, TouchableOpacity,} from "react-native";
  ...
  return (
    <View className="flex-1 bg-white">
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        {/* Image Carousel */}
        <View className="realtive h-[450px] bg-gray-100 mb-6">
          <ScrollView horizontal pagingEnabled showsHorizontalScrollIndicator={false} scrollEventThrottle={16}
            onScroll={(e) => { const slide = Math.ceil( e.nativeEvent.contentOffset.x / e.nativeEvent.layoutMeasurement.width,
              );
          ...
          <TouchableOpacity onPress={() => router.back()} className="w-10 h-10 bg-white/80 rounded-full items-center justify-center"
            ...
            <TouchableOpacity
              ...
              className="w-10 h-20 bg-white/80 rounded-full items-center justify-center"  >
      ...
      <View className="px-5">
          ...
          <View className='flex-row justify-between items-start mb-2'>
            ...
            <View className='flex-row justify-between items-start mb-2'>
              ...
              <Text className='text-sm font-bold m1-1'>4.6</Text>
               <Text className='text-sm font-bold m1-1'>85</Text>
            </View>
          </View>
    ...
    <View className='absolute bottom-0 left-0 flex-row right-0 p-4 bg-white border-t border-gray-100'>
     ...
     <TouchableOpacity onPress={()=> router.push("/(tabs)/cart")} className='w-1/5 py-3 flex-row justify-center relative'>
      ...
      <View className='absolute top-2 right-4 size-4 z-10 bg-black rounded-full justify-center items-center'>
       <Text className='text-white text-[90px]'>{itemCount}</Text>
      </View>
